import sys
import os
from nanoAPI.utils import msg, err
from nanoAPI.core.errors import ManagerArgumentError
...
def runner(app):
    ...
    if command == "run":
        if param == "db":
            ...
            os.system(
                f"gunicorn {main_file}:{param} {app.gunicorn_config()}")
        ...
        print(
            msg("MSG", f"Running server at *http://{app.host}:{app.port}*\n\t\bUse *CTRL+C* to stop the server"))
